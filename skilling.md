...
### Direct problems
   ...
   2. In free-to-play, there are no 2t weapons which players can equip. Describe a method on a PVP world using only flinching to 2t rune essence. Is this the fastest possible method with flinching? <!-- long bow on rapid, alt attacking every 4 ticks; no, flinch with melee weapon to move same tick (thanks bea) -->
   ...
   4. Continuing from problem 3, what's the best method when the attacking npc has other attack speeds, such as 2 ticks, 3 ticks, or 5 ticks? <!-- 2t, 3t, 2.5t -->
   ...
   8. Describe two distinct methods for 3t swordfish, where there's a roll once every three ticks. At most one method should involve attacking. <!-- 1) we have a 2t or 3t wep and alt has a 3t wep, 2) we click fish spot a tick later than for 3t barb -->
   9. Without varrock armour 4 or the mining cape, is it possible to gather two runite ore in one tick while mining a runite rock? Does having expert mining gloves change the answer? <!-- maybe with mining gloves, but, no, the successful roll stall blocks it -->
   10. Describe how to get both a gem and an ore in the same tick from an iron rock. <!-- by getting a double roll, having a glory on helps rng -->
   11. During 4t gem mining with a rune pickaxe, if moving between rocks every four ticks, explain why failing to receive a gem at one rock will force the next rock to also not yield a gem. Describe a more efficient variant of this gem mining method. <!-- the skilling timer delay on tick 4 (since not stalled when unsuccessful) makes it so we're "moving late" like in 3t4g, a fix is to ground click in case not stalled or use addy pick for fix or just try to herbtar+move on tick3 and tick4 -->
   12. Is it possible to cook one shark and one swordfish in the same tick? What about two sharks in the same tick? Can three fish be cooked in the same tick? <!-- a: yes by waiting 3t after interface opens then selecting option+using fire on fire (ie interacting) in same tick; b: Same methods works here (2 shark, 1 sword in inventory), but naively using both on stove makes only last one cook (since cook happens in interaction); c: No cooks can happen in client input, stall end, weak queue, or interaction. first 2 can't happen together (so at most 3) but weak queue gets interrupted by anything (so at most 2) -->
...
### Exploratory problems
   ...
   14. What's the fewest possible ticks between two casts of bones to bananas? <!-- can cast once per tick, have to pick up and cast in same tick and also drop when needed. note, with gravestones, can now get bones from client input and cast without closing gravestone interface (so answer is now zero, was one) -->
   ...
   18. At motherlode mine, describe why it's possible to receive a gem one tick after a pay-dirt without any tick manipulation. Conversely, describe why it's possible to receive a pay-dirt one tick after a gem when using tick manipulation. <!-- (a) it can happen with some rng while using a drag or crystal pick. Rolling a gem goesn't give a 1t stall but rolling a pay dirt does. This means you can T4: stall end (receive pay dirt), interact (set st=1), T5: st=0, interact (get gem). (b) can happen like problem 10 except there's a 1t stall before receiving the pay dirt. -->
   19. The first pick from an herb patch uses the normal queue. Describe how this makes possible the method "1.5t herb picking" using Henke's model. 

### Open ended problems

   20. Tree fishing is an interesting method in part because it doesn't involve any special items or attacking. Are there any other skilling methods which rely on having an average number of ticks per roll reduced without any special items or attacking? <!-- addy pick 2t ess or teak/mahog 2t and clicking net spot every 5 ticks and dropping chin trap and picking up to start 4t trap resetting and cook eat fishing -->
   21. In [2014](https://www.youtube.com/watch?v=OjusYIftpZw), [2015](https://www.youtube.com/watch?v=Jn-vf4RMwww), and [early 2017](https://www.youtube.com/watch?v=XpCMUJzCL0M), the skilling community put out videos displaying their best methods. Which mechanics were discovered in between those years? Have any new mechanics been discovered since early 2017?
   22. What do you wish would have been presented differently in this guide? Please feel free to let us know.
