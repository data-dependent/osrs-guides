...
Here we put in some fun problems to help solidify the previous material.
   ...
   9. Describe two distinct methods for 3t swordfish, where there's a roll once every three ticks. At most one method should involve attacking.
   10. Without varrock armour 4 or the mining cape, is it possible to gather two runite ore in one tick while mining a runite rock?
   11. During 4t gem mining, if moving between rocks every four ticks, explain why failing to receive a gem at one rock will force the next rock to also not yield a gem. Describe a variant of the method which fixes this problem.
   12. The method for 2t sharks we presented relied crucially on being adjacent to a bank. Without a bank, is there any way to cook a shark once every two ticks? When next to a bank, which method would you prefer?
   13. Is it possible to cook one shark and one swordfish in the same tick? What about two sharks in the same tick?
   14. What do you wish would have been presented differently in this guide?

<!-- SOME POSSIBLE SOLUTIONS:: 3: one per tick, have to pick up, and cast in same tick and drop when needed 4: long bow on rapid, alt attacking every 4 ticks, 5: 2.66t, 5: 2t, 3t, 2.5t, 7: addy pick 2t ess or teak/mahog 2t and clicking net spot every 5 ticks, 8: no, the interaction immediately before and on the xp drop tick are used by the fishing spot and so there's only one interaction slot open, 9: 1)we have a 2t or 3t wep and alt has a 3t wep, 2) we click fish spot a tick later than for 3t barb, 10: no, the successful roll stall blocks it, 11: the skilling timer delay on tick 4 (since not stalled when unsuccessful) make it so we're "moving late" like in 3t4g, 12: drop cooking, which is worse cuz it takes time to bank and drop, 13a: yes by waiting 3t after interface opens then selecting option+interacting in same tick, 13b: Same methods works here (2 shark, 1 sword in inventory), but naively using both on stove makes only last one cook (since cook happens in interaction) -->
