...
Here we put in some fun problems to help solidify the previous material.
   ...
   3. What's the most number of times that bones to bananas can be cast in 29 ticks?
   ...
   7. Tree fishing is an interesting method in part because it doesn't involve any inventory actions or attacking. Are there any other skilling actions which can have their average number of ticks per roll reduced without any inventory actions or attacking?
   8. After describing cut-eat barbarian fishing, an extended method was briefly described involving additionally picking up fish from the ground to cut for more cooking experience. Is it possible to pick up two fish from the ground within the three tick rhythm of the method?
   9. Is it possible to use a dragon/crystal harpoon spec during 2t swordfish without losing ticks? Is it possible to get a lamp from a genie without losing ticks? (Note, while moving between spots doesn't count.)
   ...
   11. Describe two distinct methods for 3t swordfish, where there's a roll once every three ticks. At most one method should involve attacking.
   12. Without varrock armour 4 or the mining cape, is it possible to gather two runite ore in one tick while mining a runite rock?
   13. During 4t gem mining with a rune pickaxe, if moving between rocks every four ticks, explain why failing to receive a gem at one rock will force the next rock to also not yield a gem. Describe a more efficient variant of this gem mining method.
   14. The method for 2t sharks we presented relied crucially on being adjacent to a bank. Without a bank, describe a way to cook a shark once every two ticks. When next to a bank, which method would you prefer?
   15. Is it possible to cook one shark and one swordfish in the same tick? What about two sharks in the same tick?
   16. What do you wish would have been presented differently in this guide?

<!-- SOME POSSIBLE SOLUTIONS:: 3: one per tick, have to pick up and cast in same tick and also drop when needed 4: long bow on rapid, alt attacking every 4 ticks, 5: 2.66t, 5: 2t, 3t, 2.5t, 7: addy pick 2t ess or teak/mahog 2t and clicking net spot every 5 ticks and dropping chin trap and picking up to start 4t trap resetting, 8: no, the interaction immediately before and on the xp drop tick are used by the fishing spot and so there's only one interaction slot open, 9: Yes, weapon speed doesn't matter when not being hit and then change back happens in client input before being hit, but we can't talk to genie without losing ticks since that uses up an interaction 11: 1)we have a 2t or 3t wep and alt has a 3t wep, 2) we click fish spot a tick later than for 3t barb, 12: no, the successful roll stall blocks it, 13: the skilling timer delay on tick 4 (since not stalled when unsuccessful) make it so we're "moving late" like in 3t4g, a fix is to ground click in case not stalled or use addy pick for ~90% fix, 14: drop cooking, which is worse cuz it takes time to bank and drop, 15a: yes by waiting 3t after interface opens then selecting option+interacting in same tick, 15b: Same methods works here (2 shark, 1 sword in inventory), but naively using both on stove makes only last one cook (since cook happens in interaction) -->
